...
class RaftCore {
  ...
  handleAppendEntries(args) {
    const currentTermBefore = this.nodeState.currentTerm;
    const result = this.heartbeat.handleAppendEntries(args);

    // Reset election timer on any AppendEntries from a leader with current or higher term
    if (args.term >= currentTermBefore) {
      this._startElectionTimer();
      this.stateMachine.becomeFollower(args.term);
    }

    // Update persistent term state when a higher term is observed
    if (args.term > currentTermBefore) {
